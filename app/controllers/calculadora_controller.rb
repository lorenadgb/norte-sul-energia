class CalculadoraController < ApplicationController
  ...
  def orcamento
    if request.get?
      ...
      NotificationMailer.envia_pdf_cliente(@calculadora, 'Norte Sul Energia - Orçamento',pdf.to_pdf).deliver!
      NotificationMailer.envia_pdf_empresa(@calculadora, 'Norte Sul Energia - Orçamento', pdf.to_pdf).deliver!
  ...
  def calculadora_params
    params.require(:calculadora).permit(:rede_eletrica, :estado, :cidade, :local, :ajuste, :media, :nome, :email, :telefone, :source) if params[:calculadora]
  end
